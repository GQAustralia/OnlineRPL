				...
				function handle() {
					...
					if ( validator.settings.submitHandler ) {
						if (validator.submitButton) {
							...
							hidden = $("<input type='hidden'/>").attr("id", validator.submitButton.id).val(validator.submitButton.value).appendTo(validator.currentForm);
						}
...
$.extend($.validator, {
	...
	messages: {
		...
		phone: "Please enter only digits, dashes and must be 10 numbers.",
		fullname: "Invalid name. Only alphabets, numbers and spaces are allowed.",
	...
	methods: {
		...
		phone: function(value, element) {
			//return this.optional(element) || /^\d{9}([0-9]|[\/\-])$/.test(value);			
			//return this.optional(element) || /^\d{9}([0-9])$/.test(value);			
			return this.optional(element) || (value.match(/^((\+){0,1}\d\s)?\s*[\. +]?\(?([0-9]{0,2})\)?\s*[\. -]?\(?([0-9]{3})\)?\s*[\. -]?\s*([0-9]{3})\s*[\. -]?\s*([0-9]{4})\s?((ext|x)\s*\.?:?\s*([0-9]+))?$/));			
		},
		// http://docs.jquery.com/Plugins/Validation/Methods/fullname
		fullname: function(value, element) {
			return this.optional(element) || /^[A-Za-z]|[^A-Za-z\u00E0\u00E8\u00E9\u00EC\u00F2\u00F3\u00F9]+$/.test(value);
		},
