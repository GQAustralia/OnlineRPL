    ...
    if (to != '' && msgBody != '') {
        ...
        $.ajax({
            ...
            success:function(result) {
                ...
                $('#message_popup').modal('hide');
                $("body #status-message").css("display", "block");
                $("body .status-message").html('<div class="gq-id-files-upload-success-text alert alert-success" style="display: block;"><h2> Sent message successfully</h2></div>').fadeOut(5000);
                window.scrollTo(0, 0);
            }
        });
    }
